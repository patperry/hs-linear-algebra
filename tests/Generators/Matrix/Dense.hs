...
matrix :: (Elem e, Arbitrary e) => (Int,Int) -> Gen (Matrix (m,n) e)
...
instance (Arbitrary e, Elem e) => Arbitrary (Matrix (m,n) e) where
...
data MatrixAt m n e =
    MatrixAt (Matrix (m,n) e)
             (Int,Int)
    deriving (Show)
instance (Arbitrary e, Elem e) => Arbitrary (MatrixAt m n e) where
    arbitrary = matrixSized $ \k -> do
...
instance (Arbitrary e, Elem e) => Arbitrary (SubMatrix m n e) where
    arbitrary = matrixSized $ \k -> do
        m <- choose (0,k)
        n <- choose (0,k)
...
instance (Arbitrary e, Elem e) => Arbitrary (MatrixMV m n e) where
    arbitrary = do
        a <- arbitrary
        x <- vector (numCols a)
        return $ MatrixMV a x
...
instance (Arbitrary e, Elem e) => Arbitrary (MatrixMVPair m n e) where
    arbitrary = do
...
instance (Arbitrary e, Elem e) => Arbitrary (MatrixMM m n k e) where
...
instance (Arbitrary e, Elem e) => Arbitrary (MatrixMMPair m n k e) where
    arbitrary = do
